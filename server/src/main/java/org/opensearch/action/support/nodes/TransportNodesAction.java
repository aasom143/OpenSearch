...
//                    Thread.sleep(1000);
                    if (task instanceof CancellableTask){
                        logger.info("Running for node with timeout: {}, {}, {}", ((CancellableTask) task).getCancellationTimeout(), nodeId, ((CancellableTask) task).isCancelled());}
                    if (task instanceof CancellableTask && ((CancellableTask) task).isCancelled()){
                        logger.info("cancelled task with reason:");
