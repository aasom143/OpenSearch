...
use datafusion::catalog::{TableProviderFactory, Session, TableProvider};
...
pub async fn execute_query_with_cross_rt_stream(
    ...
    match &table_source {
        TableSource::IcebergS3 { .. } | TableSource::IcebergGlue { .. } | TableSource::IcebergS3Tables { .. } => {
            std::env::set_var("AWS_REGION", "us-west-2");
            std::env::set_var("AWS_ACCESS_KEY_ID", "<access_key_id>");
            std::env::set_var("AWS_SECRET_ACCESS_KEY", "<secret_key_id>");
    ...
    let provider: Arc<dyn TableProvider> = match &table_source {
        ...
        TableSource::IcebergS3Tables { table_bucket_arn, database_name, s3tables_options } => {
            log_info!("[FLOW] Registering table from Iceberg S3 Tables: bucket={}, database={}, table={}",
                     table_bucket_arn, database_name, table_name);
