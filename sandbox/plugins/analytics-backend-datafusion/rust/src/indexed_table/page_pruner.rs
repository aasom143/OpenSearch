...
impl PagePruner {
    pub fn new(schema: &SchemaRef, metadata: Arc<ParquetMetaData>, seg_arrow_schema: SchemaRef) -> Self {
        Self {
            schema: schema.clone(),
            metadata,
            seg_arrow_schema,
        ...
        match node {
            BoolNode::And(children) => {
                let annotated_children: Vec<_> = children
                    .iter()
                    .map(|c| Self::build_from_bool_node(c, leaf_predicates, metadata, schema, rg_indices, seg_arrow_schema))
                    .collect();
            ...
            BoolNode::Not(child) => {
                let annotated_child = Self::build_from_bool_node(child, leaf_predicates, metadata, schema, rg_indices, seg_arrow_schema);
...
mod tests {
    ...
    fn missing_column_does_not_disable_other_columns() {
        ...
        .unwrap();
        let pruner = PagePruner::new(&predicate_schema, meta.metadata().clone(), predicate_schema.clone());
    ...
    fn stats_prune_tree_five_leaf_shape() {
        ...
        let spt = StatsPruneTree::build_from_bool_node(
            &tree, &leaf_predicates, &arc_meta, &schema, &rg_indices, &schema,
        );
