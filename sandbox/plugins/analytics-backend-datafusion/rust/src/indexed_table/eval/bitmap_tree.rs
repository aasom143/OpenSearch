    ...
    match node {
        ...
        ResolvedNode::Or(children) => {
            ...
            for (arr_index, &val) in indices.iter().enumerate() {
                ...
                // Short circuit: OR candidate superset saturated. The
                // candidate result is already the full universe so further
                // children can't widen it. However, Predicate bitmaps are
                // supersets — refinement evaluates them exactly and may
                // yield fewer bits, so remaining Collector bitmaps are
                // still needed for a correct OR at refinement time.
